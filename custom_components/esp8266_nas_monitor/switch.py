...
async def async_setup_entry(
    ...
    """Set up ESP8266 NAS Monitor switch."""
    data = entry.data
    ...
    coordinator = EspNasMonitorCoordinator(hass, api)
    await coordinator.async_config_entry_first_refresh()

    async_add_entities([EspNasMonitorScreenSwitch(entry, coordinator, api)])


class EspNasMonitorCoordinator(DataUpdateCoordinator[dict[str, Any]]):
...
class EspNasMonitorScreenSwitch(SwitchEntity):
    """Switch entity for ESP8266 NAS Monitor screen."""
    ...
    def __init__(
        self,
        entry: ConfigEntry,
        coordinator: EspNasMonitorCoordinator,
        api: EspNasMonitorApi,
    ) -> None:
        self._entry = entry
